...
// Define the secrets - support for dual-key strategy
...
// Optional: Production-specific secrets (if different from dev)
...
async function initialize() {
  ...
  try {
    ...
    console.log('🔐 Loading APNs credentials...');
    console.log('Available env vars:', Object.keys(process.env).filter(k => k.includes('APNS')));
    ...
    // Load production credentials if available (dual-key strategy)
    config.apnsKeyIdProd = process.env.APNS_KEY_ID_PROD?.trim() || config.apnsKeyId;
    ...
    console.log('- Dev Key length:', config.apnsKey ? config.apnsKey.length : 'N/A');
    console.log('- Prod Key length:', config.apnsKeyProd ? config.apnsKeyProd.length : 'N/A');
    
    if (!config.apnsKey) {
      console.error('❌ [Initialize] APNS_AUTH_KEY not found in environment');
      console.error('Available env vars:', Object.keys(process.env).filter(k => k.includes('APNS')));
      throw new Error('APNs auth key not found in environment');
    }
    
    // Validate the auth key format
    if (!config.apnsKey.includes('BEGIN PRIVATE KEY')) {
      console.error('❌ [Initialize] APNS_AUTH_KEY appears to be invalid format');
    ...
    console.log('✅ [Initialize] Successfully loaded APNs configuration');
    console.log('- Key ID:', config.apnsKeyId);
    console.log('- Team ID:', config.apnsTeamId);
    console.log('- Topic:', config.apnsTopic);
    console.log('- Key format valid:', config.apnsKey.includes('BEGIN PRIVATE KEY'));
    
    initialized = true;
  } catch (error) {
    console.error('❌ [Initialize] Failed to initialize:', error.message);
...
async function generateAPNsToken(useProduction = false) {
  ...
  try {
    ...
    console.log(`🔑 Generated ${useProduction ? 'PRODUCTION' : 'DEVELOPMENT'} JWT token (first 20 chars):`, token.substring(0, 20) + '...');
    return token;
  } catch (error) {
    console.error(`Failed to generate APNs JWT token for ${useProduction ? 'production' : 'development'}:`, error);
...
async function sendLiveActivityUpdate(pushToken, activityId, contentState, dismissalDate = null, topicOverride = null, preferredEnvironment = 'auto') {
  ...
  if (!contentState.startTime || !contentState.endTime) {
    console.error('❌ [sendLiveActivityUpdate] Invalid contentState:', contentState);
    throw new Error('contentState must have startTime and endTime');
  }
  ...
  const toISOString = (dateValue) => {
    ...
    try {
      ...
      if (typeof dateValue === 'number') {
        ...
        console.error('Invalid timestamp conversion:', dateValue);
        return null;
      }
      ...
      if (dateValue._seconds !== undefined) {
        ...
        console.error('Invalid Firestore timestamp seconds:', dateValue._seconds);
        return null;
      }
      ...
      if (dateValue.seconds !== undefined) {
        ...
        console.error('Invalid timestamp seconds:', dateValue.seconds);
        return null;
      }
  ...
  if (contentState.startedAt !== undefined) {
    console.log('🆕 Using new simplified Live Activity format');
    
    // New format - convert to ISO strings
    const startedAtISO = toISOString(contentState.startedAt);
    if (!startedAtISO) {
      console.error('❌ Failed to convert startedAt timestamp, using current time');
      convertedContentState.startedAt = new Date().toISOString();
    } else {
      convertedContentState.startedAt = startedAtISO;
    }
    
    convertedContentState.pausedAt = contentState.pausedAt ? toISOString(contentState.pausedAt) : null;
    convertedContentState.duration = contentState.duration || 300; // Default 5 minutes
    
    // Add calculated legacy fields for backward compatibility
    const startedAtDate = convertTimestamp(contentState.startedAt) || new Date();
    const duration = convertedContentState.duration;
    
    convertedContentState.startTime = convertedContentState.startedAt;
    const endTimeISO = toISOString(new Date(startedAtDate.getTime() + duration * 1000));
    convertedContentState.endTime = endTimeISO || new Date(Date.now() + duration * 1000).toISOString();
    convertedContentState.lastUpdateTime = new Date().toISOString();
    
    // Calculate elapsed and remaining times for legacy compatibility
    const now = new Date();
    if (contentState.pausedAt) {
      // If paused, calculate elapsed time from startedAt to pausedAt
      const pausedAtDate = convertTimestamp(contentState.pausedAt) || now;
      convertedContentState.elapsedTimeAtLastUpdate = Math.max(0, (pausedAtDate.getTime() - startedAtDate.getTime()) / 1000);
      convertedContentState.remainingTimeAtLastUpdate = Math.max(0, duration - convertedContentState.elapsedTimeAtLastUpdate);
    } else {
      // If running, calculate elapsed time from startedAt to now
      convertedContentState.elapsedTimeAtLastUpdate = Math.max(0, (now.getTime() - startedAtDate.getTime()) / 1000);
      convertedContentState.remainingTimeAtLastUpdate = Math.max(0, duration - convertedContentState.elapsedTimeAtLastUpdate);
    }
  } else {
    // Legacy format - convert all timestamps to ISO strings
    console.log('🔄 Using legacy Live Activity format');
    ...
    if (!startTimeISO || !endTimeISO) {
      console.error('❌ Failed to convert legacy timestamps, using defaults');
      const now = new Date();
  ...
  console.log('📋 Converted contentState for iOS:', JSON.stringify(convertedContentState, null, 2));
  ...
  const payload = {
    'aps': {
      ...
      'alert': {
        'title': contentState.methodName || 'Timer',
  ...
  for (const environment of environmentsToTry) {
    ...
    console.log(`📱 Attempting ${environment.toUpperCase()} environment...`);
    console.log(`  Host: ${apnsHost}`);
    console.log(`  Key ID: ${useProduction ? config.apnsKeyIdProd : config.apnsKeyId}`);
    
    let token;
    try {
      token = await generateAPNsToken(useProduction);
    } catch (error) {
      console.error(`❌ Failed to generate ${environment} APNs token:`, error.message);
      lastError = error;
    ...
    try {
      const result = await new Promise((resolve, reject) => {
        ...
        try {
          ...
          console.error('❌ Failed to connect to APNs:', error.message);
        ...
        req.on('end', () => {
          ...
          if (statusCode === 200) {
            console.log(`✅ Live Activity update sent successfully using ${environment} environment`);
            resolve({ 
              success: true, 
            ...
            if (statusCode === 400 && responseBody.includes('BadDeviceToken')) {
              console.error(`❌ BadDeviceToken in ${environment} - Token/server mismatch`);
            } else if (statusCode === 403) {
              console.error(`❌ 403 Forbidden in ${environment} - Invalid authentication`, errorInfo);
            } else if (statusCode === 410) {
              console.error(`❌ 410 Gone - Token is no longer valid`);
            }
        ...
        req.on('error', (error) => {
          console.error(`❌ APNs request error in ${environment}:`, error.message);
          client.close();
        ...
        try {
          ...
          console.error(`❌ Failed to send APNs request in ${environment}:`, error.message);
          client.close();
      ...
      console.log(`❌ ${environment} attempt failed:`, error.message);
      lastError = error;
      
      // Check if we should skip the retry
      if (error.message.includes('403') || error.message.includes('InvalidProviderToken')) {
        console.log(`⚠️  Skipping retry for 403 error - key issue needs to be resolved`);
        break;
      }
...
exports.updateLiveActivity = onCall(
  { 
    ...
    secrets: [
      ...
      apnsTeamIdSecret, 
      apnsTopicSecret,
      apnsAuthKeyProdSecret,  // Optional production key
      apnsKeyIdProdSecret     // Optional production key ID
    ],
    consumeAppCheckToken: false  // Disable App Check for Live Activity updates
  },
  async (request) => {
    console.log('📥 updateLiveActivity called with:', {
    ...
    if (!activityId || !contentState) {
      console.error('❌ Missing required parameters:', { activityId, hasContentState: !!contentState });
    ...
    console.log('📋 Received contentState:', JSON.stringify(contentState, null, 2));
    ...
    try {
      ...
      if (config.apnsTopic && config.apnsTopic.includes('.push-type.liveactivity')) {
        ...
        console.log(`📱 Using configured topic: ${topicOverride}`);
      ...
      console.log(`🔧 APNs Environment Detection:
      ...
      console.error('❌ [updateLiveActivity] Error:', error.message);
      console.error('Stack trace:', error.stack);
...
exports.updateLiveActivityTimer = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      console.error('❌ [updateLiveActivityTimer] Error:', error.message);
      console.error('Stack trace:', error.stack);
      console.error('Error details:', {
        activityId,
        action,
...
exports.testAPNsConnection = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          devClient.on('connect', () => {
            console.log('✅ Connected to development APNs');
            results.development.connection = 'Connected successfully';
            devClient.close();
            resolve();
          });
          devClient.on('error', (err) => {
            console.error('❌ Development APNs error:', err.message);
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          prodClient.on('connect', () => {
            console.log('✅ Connected to production APNs');
            results.production.connection = 'Connected successfully';
            prodClient.close();
            resolve();
          });
          prodClient.on('error', (err) => {
            console.error('❌ Production APNs error:', err.message);
      ...
      console.error('❌ testAPNsConnection error:', error);
      return {
        success: false,
        error: error.message,
        stack: error.stack
