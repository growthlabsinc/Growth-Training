    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: AppTheme.Layout.spacingM) {
                ...
                // Debug buttons for Live Activity testing
                VStack(spacing: 8) {
                    Button(action: {
                ...
                if isMultiMethod, 
                   ...
                   let nextMethod = sessionVM.nextMethod {
                    UpNextPreview(nextMethod: nextMethod)
                        ...
                        .animation(.spring(response: 0.5, dampingFraction: 0.8), value: sessionVM.showUpNextPreview)
                }
        ...
        // Story 7.3: Apply a red tint if overexertion warning is active
        .overlay(
        ...
        .onChange(of: viewModel.wasStoppedFromLiveActivity) {
            // When the timer is stopped from Live Activity, trigger the completion flow
            if viewModel.wasStoppedFromLiveActivity {
                // Reset the flag
