...
class LiveActivityPushUpdateService {
    ...
    private func sendPushUpdate(activityId: String) {
        ...
        if activity.content.state.isPaused {
        ...
        Task {
            do {
                // Calculate new end time for countdown timers
                var endTime: String? = nil
                if activity.content.state.sessionType == .countdown {
                    // For countdown, we need to maintain the original end time
                    endTime = ISO8601DateFormatter().string(from: activity.content.state.endTime)
                }
