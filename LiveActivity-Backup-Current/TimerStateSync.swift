    ...
    func startListeningForStateChanges(completion: @escaping (TimerActivityAttributes.ContentState?) -> Void) {
        ...
        timerStateListener = db.collection("activeTimers").document(userId)
            .addSnapshotListener { documentSnapshot, error in
                ...
                // Parse content state - using correct TimerActivityAttributes.ContentState constructor
                if let startedAtTimestamp = contentStateData["startedAt"] as? Timestamp,
                   let duration = contentStateData["duration"] as? TimeInterval,
                   let methodName = contentStateData["methodName"] as? String,
                   let sessionTypeStr = contentStateData["sessionType"] as? String {
                    
                    let sessionType = SessionType(rawValue: sessionTypeStr) ?? .countup
                    let pausedAtTimestamp = contentStateData["pausedAt"] as? Timestamp
                    
                    let contentState = TimerActivityAttributes.ContentState(
                        startedAt: startedAtTimestamp.dateValue(),
                        pausedAt: pausedAtTimestamp?.dateValue(),
                        duration: duration,
                        methodName: methodName,
                        sessionType: sessionType
                    )
