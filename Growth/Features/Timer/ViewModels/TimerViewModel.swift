...
class TimerViewModel: ObservableObject {
    ...
    private func setupBindings() {
        ...
        NotificationCenter.default
            ...
            .sink { [weak self] _ in
                ...
                // Check for pending timer completion
                self?.checkForPendingTimerCompletion()
            }
            .store(in: &cancellables)
        
        // Check for pending completion on init as well
        ...
        NotificationCenter.default
            ...
            .sink { [weak self] notification in
                ...
                Logger.info("TimerViewModel: Received timerStoppedFromLiveActivity notification", logger: AppLoggers.timer)
                
                // Extract values from notification userInfo
    ...
    func checkForPendingTimerCompletion() {
        ...
        if let completionData = sharedDefaults?.dictionary(forKey: "pendingTimerCompletion"),
           ...
           let startTimeInterval = completionData["startTime"] as? TimeInterval,
           let timestamp = completionData["timestamp"] as? TimeInterval {
            
            // Only process if the data is recent (within 30 seconds)
            let dataAge = Date().timeIntervalSince1970 - timestamp
            if dataAge < 30 && elapsedTime > 0 {
                Logger.info("TimerViewModel: Found pending timer completion from \(dataAge)s ago", logger: AppLoggers.timer)
                ...
                // For now, we'll just set the flag to trigger completion
                // The method info will be pulled from the saved method name
                self.wasStoppedFromLiveActivity = true
                Logger.info("TimerViewModel: Set wasStoppedFromLiveActivity = true from pending data", logger: AppLoggers.timer)
                Logger.info("TimerViewModel: Method name from Live Activity: \(methodName)", logger: AppLoggers.timer)
                
                // Clear the pending data after processing
                sharedDefaults?.removeObject(forKey: "pendingTimerCompletion")
                sharedDefaults?.synchronize()
            } else {
                Logger.info("TimerViewModel: Pending timer completion data is too old (\(dataAge)s) or invalid, clearing", logger: AppLoggers.timer)
                // Clear old data
                sharedDefaults?.removeObject(forKey: "pendingTimerCompletion")
                sharedDefaults?.synchronize()
            }
