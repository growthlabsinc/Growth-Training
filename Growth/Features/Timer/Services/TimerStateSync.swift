...
class TimerStateSync {
    ...
    func updatePauseState(isPaused: Bool, pausedAt: Date? = nil, adjustedStartedAt: Date? = nil) {
        ...
        // Get the current activity ID from stored data
        db.collection("activeTimers").document(userId).getDocument { [weak self] snapshot, error in
            guard let self = self,
                  let data = snapshot?.data(),
                  let activityId = data["activityId"] as? String else {
                print("❌ TimerStateSync: No active timer found to update pause state")
                return
            }
            ...
            self.db.collection("activeTimers").document(userId).setData(updateData, merge: true) { error in
                if let error = error {
                    ...
                    // Now call the cloud function with the correct action
                    let action = isPaused ? "pause" : "resume"
                    print("🔄 TimerStateSync: Calling cloud function with action: \(action)")
                    self.sendLiveActivityUpdate(activityId: activityId, action: action)
                }
            }
        }
    }
    ...
    func startListeningForStateChanges(completion: @escaping (TimerActivityAttributes.ContentState?) -> Void) {
        ...
        timerStateListener = db.collection("activeTimers").document(userId)
            .addSnapshotListener { documentSnapshot, error in
                ...
                // Parse content state - handle both new and legacy field names
                let startedAt: Date
                let duration: TimeInterval
                
                // Check for new field names first, fall back to legacy
                if let startedAtStamp = contentStateData["startedAt"] as? Timestamp {
                    startedAt = startedAtStamp.dateValue()
                } else if let startTimeStamp = contentStateData["startTime"] as? Timestamp {
                    startedAt = startTimeStamp.dateValue()
                } else {
                    completion(nil)
                    return
                }
                
                // Calculate duration from endTime or use stored duration
                if let durationValue = contentStateData["duration"] as? TimeInterval {
                    duration = durationValue
                } else if let endTimeStamp = contentStateData["endTime"] as? Timestamp {
                    duration = endTimeStamp.dateValue().timeIntervalSince(startedAt)
                } else {
                    duration = 0
                }
                
                guard let methodName = contentStateData["methodName"] as? String,
                      let sessionTypeStr = contentStateData["sessionType"] as? String else {
                    completion(nil)
                    return
                }
                
                let sessionType = SessionType(rawValue: sessionTypeStr) ?? .countdown
                let pausedAt = (contentStateData["pausedAt"] as? Timestamp)?.dateValue()
