...
// Deploy Angion Methods with full multi-step data
exports.deployAngionMethods = onCall(async (request) => {
  try {
    ...
    const angionMethod1 = {
      ...
      description: "Venous-focused technique to improve blood flow. Step-by-step foundational vascular training.",
      instructionsText: "A comprehensive step-by-step guide to performing the Angion Method 1.0 for optimal venous circulation improvement.",
      steps: [
        {
          stepNumber: 1,
      ...
      equipmentNeeded: [
        "Water-based lubricant (non-paraben) or silicone-based lubricant (preferred)",
        "Clean towel",
        "Timer or stopwatch"
      ],
      estimatedDurationMinutes: 30,
      categories: ["Angion", "Foundation", "Vascular"],
      isFeatured: true,
      safetyNotes: "Never perform while seated - this can restrict blood flow. Stop immediately if pain occurs. Loss of erection is normal initially as your vascular system adapts. Focus on the technique rather than maintaining erection.",
      benefits: [
        "Improved venous circulation",
        "Better resting fullness",
    ...
    batch.set(db.collection('growth_exercises').doc('angion_method_1_0'), angionMethod1, { merge: true });
    batch.set(db.collection('growth_exercises').doc('angio_pumping'), angioPumping, { merge: true });
    
    await batch.commit();
    
    console.log("✅ Methods deployed successfully!");
    
    return { 
      success: true, 
      message: 'Angion methods deployed successfully with full multi-step instructions',
      methods: [
        {
          id: 'angion_method_1_0',
          title: 'Angion Method 1.0',
          steps: angionMethod1.steps.length
        },
        {
          id: 'angio_pumping',
          title: 'Angio Pumping',
          steps: angioPumping.steps.length
        }
      ]
    };
  } catch (error) {
