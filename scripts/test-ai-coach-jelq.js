...
/**
 * Test AI Coach End-to-End - "How do I jelq?" Query
 *
 * Tests the ACTUAL AI Coach (Vertex AI + RAG) not just keyword search.
 * This simulates a real user asking the Growth Coach a question.
 *
 * Usage:
 *   GCLOUD_PROJECT=growth-training-app node scripts/test-ai-coach-jelq.js
 */
...
// Import the actual AI Coach function
  ...
  for (const test of testQueries) {
    ...
    try {
      ...
      const jelqGuideUsed = usedKnowledge && response.sources.some(s =>
        s.id === 'jelqing_technique_guide' || s.title.toLowerCase().includes('jelq')
      );
...
function generateReport(results) {
  ...
  if (successRate === 100) {
    ...
    console.log('The AI Coach successfully answers jelqing questions using the deployed knowledge base.');
  } else if (successRate >= 66) {
    console.log('✅ GOOD: Most tests passed.');
    console.log('The AI Coach can answer jelqing questions, with some areas for improvement.');
  } else {
    console.log('⚠️  NEEDS IMPROVEMENT: Many tests failed.');
    console.log('The AI Coach may not be using the jelqing knowledge base effectively.');
  }
  console.log('');
