// Script to add legal documents to Firestore using client SDK
// Run with: node scripts/add-legal-documents-client.js
...
async function addLegalDocuments() {
  ...
  console.log('Note: This script requires public write access to legalDocuments collection.');
  console.log('You may need to temporarily update Firestore rules or use the Firebase console.\n');
  
  try {
    // Try to add documents without authentication
    // This will only work if the rules allow public write (which they shouldn't in production)
    for (const [docId, docData] of Object.entries(legalDocuments)) {
      try {
        await setDoc(doc(db, 'legalDocuments', docId), docData);
