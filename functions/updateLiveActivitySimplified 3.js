 ...
 * Note: Secrets are injected into process.env when the function runs
 */
function generateAPNsToken() {
    // In v2 functions with secrets, the secret values are injected into process.env
    // The environment variable name is the same as the secret name
    const APNS_KEY = process.env.APNS_AUTH_KEY;
    const actualKeyId = process.env.APNS_KEY_ID || KEY_ID;
    const actualTeamId = process.env.APNS_TEAM_ID || TEAM_ID;
    
    if (!APNS_KEY) {
        console.error('❌ APNs key not found in environment');
        console.error('Available env vars:', Object.keys(process.env).filter(key => key.includes('APNS')));
        return null;
    }
    
    try {
        // Ensure the key is properly formatted
        let formattedKey = APNS_KEY;
        if (!APNS_KEY.includes('BEGIN PRIVATE KEY')) {
        ...
        console.log(`   Team ID: ${actualTeamId}, Key ID: ${actualKeyId}`);
        return token;
    } catch (error) {
        console.error('❌ Failed to generate JWT:', error.message);
        console.error('JWT generation details:', {
            hasKey: !!APNS_KEY,
            keyLength: APNS_KEY?.length,
            teamId: actualTeamId,
            keyId: actualKeyId
        });
        return null;
