...
async function sendLiveActivityUpdate(pushToken, activityId, contentState, dismissalDate = null, topicOverride = null, preferredEnvironment = 'auto') {
  ...
  // Convert date strings to Unix timestamps for iOS compatibility
  const convertedContentState = { ...contentState };
  
  // Helper to convert various date formats to Unix timestamp
  const toUnixTimestamp = (dateValue) => {
    if (!dateValue) return null;
    
    // If already a number, assume it's a Unix timestamp
    if (typeof dateValue === 'number') {
      // Check if it's seconds or milliseconds
      if (dateValue < 10000000000) {
        return dateValue; // Already in seconds
      } else {
        return Math.floor(dateValue / 1000); // Convert from milliseconds
      }
    }
    ...
    if (dateValue._seconds !== undefined) {
      return Number(dateValue._seconds);
    }
  ...
  // Convert all date fields to Unix timestamps
  convertedContentState.startTime = toUnixTimestamp(contentState.startTime);
  convertedContentState.endTime = toUnixTimestamp(contentState.endTime);
  
  if (contentState.lastUpdateTime) {
    convertedContentState.lastUpdateTime = toUnixTimestamp(contentState.lastUpdateTime);
  }
  
  if (contentState.lastKnownGoodUpdate) {
    convertedContentState.lastKnownGoodUpdate = toUnixTimestamp(contentState.lastKnownGoodUpdate);
  }
  
  if (contentState.expectedEndTime) {
    convertedContentState.expectedEndTime = toUnixTimestamp(contentState.expectedEndTime);
  }
