...
async function sendLiveActivityUpdate(pushToken, activityId, contentState, dismissalDate = null, topicOverride = null, preferredEnvironment = 'auto') {
  ...
  if (contentState.startedAt !== undefined) {
    ...
    // Detect event type from state changes
    let eventType = contentState.event || 'update';
    ...
    // Store event for priority determination
  ...
  if (preferredEnvironment === 'development') {
    ...
    // Auto mode: try production first for production apps
    // The environment will be properly detected based on bundle ID
    environmentsToTry = ['production', 'development'];
  }
  ...
  for (const environment of environmentsToTry) {
    ...
    try {
      const result = await new Promise((resolve, reject) => {
        ...
        // Use priority 5 (low) for regular updates to avoid throttling
        // Use priority 10 (high) only for critical state changes like pause/resume
        const eventType = contentState.event || 'update';
        const isCriticalEvent = ['pause', 'resume', 'stop', 'start'].includes(eventType);
        const apnsPriority = isCriticalEvent ? '10' : '5';
...
exports.updateLiveActivity = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      // Pass event type to use correct APNs priority
      contentState.event = eventType;
...
exports.updateLiveActivityTimer = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      // Set event type based on action for priority determination
      contentState.event = action; // Pass action as event type
      ...
      switch (action) {
        ...
        case 'resume':
          ...
          break;
        case 'stop':
          contentState.event = 'stop'; // Ensure stop gets high priority
...
exports.onTimerStateChange = onDocumentWritten(
  ...
  async (event) => {
    ...
    try {
      ...
      // Pass event type to contentState for priority determination
      contentState.event = eventType;
