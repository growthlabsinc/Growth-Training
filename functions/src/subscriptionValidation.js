...
exports.validateSubscriptionReceipt = onCall(
  ...
  async (request) => {
    const enhancedLogger = new EnhancedLogger('validateSubscriptionReceipt');
    const requestId = enhancedLogger.startRequest('validateSubscriptionReceipt', request);
    
  try {
    // Authenticate user
    if (!request.auth) {
      enhancedLogger.error('Authentication required', null, { requestId });
      enhancedLogger.endRequest(false, { error: 'unauthenticated' });
      throw new HttpsError('unauthenticated', 'User must be authenticated');
    }

    const { receiptData, forceRefresh = false, platform = 'ios' } = request.data;
    const userId = request.auth.uid;

    if (!receiptData) {
      enhancedLogger.error('Missing receipt data', null, { userId, requestId });
      enhancedLogger.endRequest(false, { error: 'invalid-argument' });
      throw new HttpsError('invalid-argument', 'receiptData is required');
    }

    // Log StoreKit validation request
    enhancedLogger.logStoreKit('VALIDATION_REQUEST', {
      receiptData,
      environment: request.data.environment || 'production',
      bundleId: request.data.bundleId,
